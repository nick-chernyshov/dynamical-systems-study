#ifndef WINDINGTHREAD_H
#define WINDINGTHREAD_H
...
struct WindingNumbers {
                                    ...
                                    Equations<state_size, parameter_size> & ode)
    {
        ...
        for (int i = 0; i < sectionArrayLength_; i++)
        {
            ...
            s2-=s;
            if (LinAlg<state_size>::norm(s2) < nearValue_ && getWindingNumber(s).empty())
                return rVal;
            rVal.push_back(s1);
        }
        return rVal;
    ...
    {
        vector<int> rVal;
        vector<int>::iterator itr = rVal.begin();
        for (int i = 0; i < sections.size(); i++)
        {
            vector<int> sectionWNSet = getWindingNumber(sections[i]);
            for (int j = 0; j < sectionWNSet.size(); j++)
